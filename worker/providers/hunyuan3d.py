"""Adaptateur Hunyuan3D — via Replicate.

Supporte text-to-3D et image-to-3D via le modèle Replicate
tencent/hunyuan3d-2. Retourne un modèle au format GLB.

Schéma Replicate :
- POST /v1/models/{owner}/{name}/predictions -> { id }
- GET  /v1/predictions/{id} -> status + output

Env : HUNYUAN3D_API_KEY (clé API Replicate)
      https://replicate.com/account/api-tokens

NOTE : le nom de modèle `tencent/hunyuan3d-2` est indicatif ; vérifier
...
def _headers() -> dict:
    return {"Authorization": f"Token {require_env('HUNYUAN3D_API_KEY')}"}
...
def _extract_output_url(data: dict) -> str | None:
    ...
    if isinstance(output, list) and output:
        ...
        if isinstance(first, dict):
            return first.get("url") or first.get("mesh")
    return None
...
def generate(model_id: str, input_: dict, timeout_ms: int) -> dict:
    ...
    `model_id` est le chemin Replicate (ex: tencent/hunyuan3d-2).
    Accepte `imageUrl` (str) ou `imageUrls` (dict vue -> URL) pour l'image-to-3D.
    """
    prompt = input_.get("prompt")
    image_url = input_.get("imageUrl")
    image_urls = input_.get("imageUrls")
    has_image = bool(image_url) or bool(image_urls)
    ...
    if has_image:
        if isinstance(image_urls, dict):
            ...
            source = image_url
        if not source:
            raise ProviderError("hunyuan3d: no image provided")
        body_input: dict = {"image": _resolve_image_url(str(source))}
    else:
        if not isinstance(prompt, str) or not prompt.strip():
            raise ProviderError("hunyuan3d: missing prompt")
        body_input = {"prompt": prompt.strip()}
    ...
    if not prediction_id:
        raise ProviderError("hunyuan3d: no prediction id")
    ...
    def extract_error(status):
        ...
        if state in ("failed", "canceled"):
            ...
            return f"hunyuan3d prediction {state}: {error}"
        return None

    model_url = poll_until_done(fetch_status, extract_done, extract_error, timeout_ms, interval_ms=5000)
    if not model_url:
        raise ProviderError("hunyuan3d: no model url in completed prediction")
